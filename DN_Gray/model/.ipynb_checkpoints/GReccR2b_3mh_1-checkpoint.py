...
class CES(nn.Module):
    def __init__(self,in_channels,num=4):
        ...
        self.c1 = CE(in_channels=in_channels)
        self.c2 = CE(in_channels=in_channels)
        self.c3 = CE(in_channels=in_channels)
    def forward(self, x):
        out = self.c1(x)
        out = self.RBS1(out)
        out = self.c2(out)
...
class CE(nn.Module):
    ...
    def forward(self, b):
        ...
        for xii,xi, wi,pi in zip(f_groups,patch_112_group_2, patch_28_group, patch_112_group):
            ...
            _, paddings = same_padding(xii, [self.ksize, self.ksize], [self.stride_2, self.stride_2], [1, 1])
            c_s = pi.shape[2]
            k_s = wi[0].shape[2]
            ...
            top_k = min(self.num_edge, yi.shape[1])
