...
class CE(nn.Module):
    def __init__(self, ksize=7, stride_1=4, stride_2=1, softmax_scale=10,shape=64 ,p_len=64,in_channels=64
        ...
        # self.soft_thr = nn.Parameter(torch.Tensor([1.0]))
        # self.soft_bias = nn.Parameter(torch.Tensor([0.0]))
...
#         print('unfold',time.time()-ts)
        ...
        for xi, wi,pi,thr,bias in zip(patch_112_group_2, patch_28_group, patch_112_group,soft_thr,soft_bias):
            ...
            mask = F.relu(yi-yi.mean(dim=1,keepdim=True)*thr.unsqueeze(1)+bias.unsqueeze(1))#(yi>yi.mean(dim=1,keepdim=True)).float()
            mask_b = (mask!=0.).float()
            yi = yi * mask
...
if __name__ == "__main__":
    data = torch.FloatTensor(size=(2,64,16,16)).normal_(0,1).cuda()
    net = CE(in_channels=64).cuda()
    out = net(data)
    print(out.shape)