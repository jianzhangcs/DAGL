from model.CA_model_v2 import ContextualAttention_Enhance
...
class merge_block(nn.Module):
    def __init__(self,in_channels,out_channels,vector_length=32,use_multiple_size=False):
...
if __name__ == "__main__":
    net = merge_block(in_channels=64,out_channels=64,vector_length=32,use_multiple_size=True).cuda()
    data = torch.zeros((2,64,8,8)).cuda()
    out = net(data)
    print(out.shape)
